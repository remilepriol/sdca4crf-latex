...
Under this assumption, the joint probability $\alpha_i(y)$ can be written as a function of its marginals $\mu_{i,c}$.
\begin{equation}
...
When we replace the joint $\alpha_i$ with the marginal $\mu_i$, we go from a size $\Y_i$, to a size $\sum_{c \in \mathcal C_i} |\Y_c|$.
...
\subsection{SDCA}
As we have seen in the introduction about structured prediction, the separation of the feature vectors is equivalent to the factorization of the primal probabilities.
