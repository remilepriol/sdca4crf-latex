...
They both are dynamic programming algorithm that can be coded in relatively few lines.

There two complicated objects in the algorithm \ref{sdca for crf} : the marginals and the features.
...
The best one is SAG-NUS* : stochastic average gradient with a mixed uniform -- non-uniform sampling scheme.
SAG-NUS* reaches a sub-optimality $10^{-4}$ on the primal in about 100 iterations.
...
When $\lambda$ gets to smaller, more reasonable values, the algorithm gradually stop converging before increasing the duality gap.
...
To investigate what really happens, we can look at a number of variables. 
In figure \ref{ocr annexes}, we look at the evolution of some of them.
...
These two variables are used in the definition of the line search.
Both of them should converge towards zero.
This is not true for the norm of the ascent direction, which do not converge for small values of $\lambda$.

The third line of plots represent the evolution of the step sizes.
...
It increases as we would expect it
...
If the algorithm was running normally, we would do that operation only every T epochs.
